...
privileged aspect TortletController_Roo_Controller {
    ...
    public String TortletController.create(@Valid Tortlet tortlet, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, tortlet);
            return "tortlets/create";
        }
        uiModel.asMap().clear();
        tortlet.persist();
    ...
    public String TortletController.createForm(Model uiModel) {
        populateEditForm(uiModel, new Tortlet());
        return "tortlets/create";
    }
    ...
    public String TortletController.update(@Valid Tortlet tortlet, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, tortlet);
    ...
    public String TortletController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        Tortlet tortlet = Tortlet.findTortlet(id);
        tortlet.remove();
        uiModel.asMap().clear();
        uiModel.addAttribute("page", (page == null) ? "1" : page.toString());
        uiModel.addAttribute("size", (size == null) ? "10" : size.toString());
        return "redirect:/tortlets";
    }
